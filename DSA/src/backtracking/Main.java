...
//        pathWithObstacles2("",board,0,0);
    ...
    static void backTrackMatrix(String p,boolean[][]maze,int row,int col){
        if(row == maze.length-1 && col == maze[0].length-1){
            System.out.println(p);
            return;
        }
        ...
        maze[row][col] = false;
        if(row<maze.length-1 ){
            backTrackMatrix(p+"D",maze,row+1,col);
        }
        if(col< maze[0].length -1 ){
            backTrackMatrix(p+"R",maze,row,col+1);
        }
        if(row>0){
            backTrackMatrix(p+"U",maze,row-1,col);
        }
        if(col>0){
